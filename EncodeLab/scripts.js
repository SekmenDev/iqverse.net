...
$('b64-paste-btn').addEventListener('click', async () => {
  ...
  if (t !== null) { $('b64-input').value = t; runBase64(); }
});
...
function urlEncode(str) {
  ...
  switch (mode) {
    ...
    case 'full':      return encodeURI(str);
    case 'rfc3986':
      return encodeURIComponent(str)
        .replace(/[!'()*]/g, c => '%' + c.charCodeAt(0).toString(16).toUpperCase());
    case 'form':
      return encodeURIComponent(str).replace(/%20/g, '+');
    default: return encodeURIComponent(str);
  }
}
...
function runUrlEncode() {
  ...
  hide('url-error');
  if (!input.trim()) { $('url-output').value = ''; return; }
  try {
    $('url-output').value = urlState.mode === 'encode'
      ? urlEncode(input)
      : urlDecode(input);
  } catch (e) {
    $('url-output').value = '';
...
document.querySelectorAll('input[name="url-mode"]').forEach(r =>
  r.addEventListener('change', runUrlEncode));
...
$('url-paste-btn').addEventListener('click', async () => {
  const t = await pasteText();
  if (t !== null) { $('url-input').value = t; runUrlEncode(); }
});
...
function runJWT() {
  ...
  try {
    ...
    let statusText = 'No exp', statusClass = 'badge warning';
    if (payload.exp) {
      if (payload.exp < now) {
        statusText = 'Expired'; statusClass = 'badge expired';
      } else if (payload.nbf && payload.nbf > now) {
        statusText = 'Not yet valid'; statusClass = 'badge warning';
      } else {
        statusText = 'Valid'; statusClass = 'badge valid';
      }
    }
...
$('jwt-paste-btn').addEventListener('click', async () => {
  const t = await pasteText();
  if (t !== null) { $('jwt-input').value = t; runJWT(); }
});
...
['file-datauri','file-linebreak-b64'].forEach(id =>
  $(id).addEventListener('change', () => fileState.file && processFile(fileState.file)));
...
$('duri-paste-btn').addEventListener('click', async () => {
  const t = await pasteText();
  if (t !== null) { $('duri-input').value = t; parseDataUri(); }
});
...
function buildDataUri() {
  ...
  if (!input) { $('duri-build-output').value = ''; return; }
...
function diffDecode(str) {
  if (diffState.mode === 'base64') {
    ...
    const bytes = new Uint8Array(raw.length);
    for (let i = 0; i < raw.length; i++) bytes[i] = raw.charCodeAt(i);
    try { return new TextDecoder('utf-8', { fatal: true }).decode(bytes); } catch { return raw; }
  } else {
...
function runDiff() {
  ...
  if (!a || !b) { toast('Enter both strings to compare', 'error'); return; }

  let da, db;
  try { da = diffDecode(a); } catch (e) {
    $('diff-error').textContent = 'String A decode error: ' + e.message;
    show('diff-error'); return;
  }
  try { db = diffDecode(b); } catch (e) {
