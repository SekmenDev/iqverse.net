...
const SECURITY_HEADERS = [
	...
	},
	{
		...
		category: "security",
		description:
			"Controls whether the page can be embedded in iframes. Protects against clickjacking attacks. Note: CSP frame-ancestors is the modern replacement.",
		check(val) {
	...
	},
	{
		...
		category: "security",
		description:
			'Legacy XSS filter for older browsers. Modern browsers use CSP instead. Setting to "0" is often recommended to avoid introducing vulnerabilities in old browsers.',
		check(val) {
...
function headerCard(r, idx) {
	...
	const mdnLink = def.mdn ? `<a href="${def.mdn}" target="_blank" style="color:var(--info);font-size:11px;text-decoration:none;">↗ MDN Docs</a>` : "";

	const rec =
		def.recommendation && status !== "pass"
...
function renderReferencePanel() {
	...
	el.innerHTML = SECURITY_HEADERS.map(
		(def) => `
...
function copyReport() {
	...
	const lines = [
		`HeaderScan Report — ${new Date().toISOString()}`,
...
function iconWarn() {
	return `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>`;
}
function iconFail() {
	return `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>`;
}
function iconInfo() {
	return `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>`;
}
