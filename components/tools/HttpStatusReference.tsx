...
const HTTP_STATUS_CODES: StatusCodeInfo[] = [
  ...
  { code: 405, phrase: 'Method Not Allowed', category: '4xx', spec: 'RFC 9110 §15.5.6', summary: 'HTTP method used is not supported for target resource.', details: 'Response must include Allow header specifying valid methods (e.g. GET, POST).' },
  ...
  { code: 502, phrase: 'Bad Gateway', category: '5xx', spec: 'RFC 9110 §15.6.3', summary: 'Server acting as gateway/proxy received invalid response from upstream server.', details: 'Common with reverse proxies like Nginx or Cloudflare when app service fails.' },
...
export default function HttpStatusReference() {
  ...
  const [activeCategory, setActiveCategory] = useState<'all' | '1xx' | '2xx' | '3xx' | '4xx' | '5xx'>('all');
          ...
          <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 20, flexWrap: 'wrap' }}>
            <div className={sharedStyles.field} style={{ margin: 0, flex: 1, minWidth: 260 }}>
              ...
              <input
                ...
                placeholder="e.g. 404 or Unauthorized or Redirect..."
              />
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 22, flexWrap: 'wrap' }}>
              {(['all', '1xx', '2xx', '3xx', '4xx', '5xx'] as const).map((cat) => (
                <button
                  key={cat}
                  type="button"
                  className={`${sharedStyles.button} ${activeCategory === cat ? sharedStyles.buttonPrimary : ''}`}
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <span style={{ fontSize: '1.4rem', fontWeight: 'bold', fontFamily: 'monospace', color: catColor }}>
                        {item.code}
                      </span>
                      <span style={{ fontSize: '1.1rem', fontWeight: 'bold' }}>{item.phrase}</span>
                    </div>
                    <span style={{ fontSize: '0.75rem', opacity: 0.6, fontFamily: 'monospace' }}>{item.spec}</span>
                  </div>

                  <div style={{ fontSize: '0.95rem', marginBottom: 6, fontWeight: 500 }}>{item.summary}</div>
                  <div style={{ fontSize: '0.85rem', opacity: 0.8, lineHeight: 1.4 }}>{item.details}</div>
                </div>
              );
            })}

            {filteredCodes.length === 0 && (
              <div style={{ textAlign: 'center', padding: 32, opacity: 0.6 }}>No HTTP status codes match your search filter.</div>
            )}
          </div>
        </div>
      </section>
